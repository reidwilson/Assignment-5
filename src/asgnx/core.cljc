...
(defn welcome [pmsg]
  (let [{_ :cmd
         arguments :args} pmsg]
    (str "Welcome " (first arguments))))


;(welcome {:cmd "welcome" :args ["foo"]})
...
(defn formatted-hours [hours]
  (let [{stime :start
         etime :end
         endloc :location} hours]
    (str "from " (format-hour stime) " to "
         (format-hour etime) " in " endloc)))
...
;(instructor-hours "tuesday")
;(instructor-hours "thursday")
;(office-hours {:cmd "office hours" :args ["tuesday"]})
;(office-hours {:cmd "office hours" :args ["monday"]})
;(contains? instructor-hours "tuesday")
...
(defn experts-register [experts topic id info]
  (action-insert [:expert (keyword topic) (keyword id)] info))
...
(defn experts-unregister [experts topic id]
 (action-remove [:expert (keyword topic) (keyword id)]))
...
(defn ask-experts [experts {:keys [args user-id]}]
 (let [topic (first args)
       msg (rest args)
       ;rem-action (action-remove [:expert (keyword topic)])
       msg-actions (action-send-msgs experts msg);needs work
       ;conversation
       insert-actions (action-inserts [:expert (keyword topic)] experts user-id)]
              ;insert-action (action-inserts [:expert (keyword topic)] experts msg)]
  (if(empty? experts)
    [[] "There are no experts on that topic."]
    [(concat msg-actions insert-actions) (experts-question-msg experts msg)])))
...
(defn create-router [routes]
  (fn [pmsg]
   (let [{command :cmd} pmsg]
    (if(contains? routes command)
      (routes command)
      (routes "default")))))
